...
function AddUser({business_code, invoice_currency,buisness_year, baseline_create_date, area_business, invoice_id, cust_number, doc_id, due_in_date, total_open_amount, posting_date, document_type, cust_payment_terms,clear_date, document_create_date, posting_id, changeHandler, submitHandler, closeHandler})
{
    ...
    return <div className = "addform">
        <form name = "forms" onSubmit = {changeHandler}>
            ...
            <Button style = {{outline:'solid',height:50 ,width:200}} onClick = {closeHandler}>Close</Button>
            <Button  style = {{marginLeft:125,outline:'solid',height:50 ,width:200}} id = "submit" type = {"submit"} value = {"Add"} onClick = {submitHandler} onClose = {submitHandler}>Add</Button>           
        </form>
        </div>
}
