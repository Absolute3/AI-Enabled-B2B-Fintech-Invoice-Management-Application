...
def predict(nulldata):
    ...
    nulldata['cust_number'] = nulldata['cust_number'] \
        .replace('CCCA02',"1") \
        .replace(['CCU001','CCU013','CCU002'],"2") \
        .replace(['CC3411','CC6000'],"3") \
        .astype(int)
    ...
    nulldata.drop(['business_code',"baseline_create_date","due_in_date","posting_date","name_customer","cust_payment_terms"],axis=1,inplace=True)
    ...
    Final = nulldata1.merge(final_result, left_index=True, right_index=True)
    Final['clear_date'] = pd.to_datetime(Final['due_in_date']) + pd.to_timedelta(Final['avg_delay'], unit='s')
    Final['avg_delay'] = Final.apply(lambda row: row.avg_delay // 86400, axis = 1)

    #bins = [0, 15, 30, 45, 60, 100]
    #bins = pd.IntervalIndex.from_tuples([(0, 15), (16, 30), (31, 45), (46, 60), (61, 100)])
    bins = [0, 16, 31, 46, 61, 1001]
    labels = ['0-15', '16-30', '31-45', '46-60', 'Greater than 60']
    Final['Aging Bucket'] = pd.cut(Final['avg_delay'], bins=bins, labels=labels, right=False)

    #Final.drop(['key_0',"avg_delay"],axis=1,inplace=True)
    Final.to_csv("Final.csv")
    Final1 = Final[['doc_id', 'clear_date', 'avg_delay', 'Aging Bucket']].copy()
    doc = Final1["doc_id"].tolist()
    aging = Final1["Aging Bucket"].tolist()
    #clear_date = Final1["clear_date"].to_list()
    #avg_delay = Final1["avg_delay"].to_list()

    a=[]
    for i in range(len(doc)):
        a.append({
            "doc_id": doc[i],
            "aging_bucket": str(aging[i]),
            #"clear_date": clear_date[i].strftime('%Y-%m-%d'),
